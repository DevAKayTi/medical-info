...
export default function LatestNews() {
  const [articles, setArticles] = useState<{ id: string; title: string; slug: string; category: string; excerpt: string; image: string; author: string; authorImage: string; publishedAt: string; readingTime: number }[]>([]);

  useEffect(() => {
    newsApi.getAll({ limit: 3, status: 'published', featured: 'true' })
      .then((res) => {
        const data = res.data.data.length > 0 ? res.data.data : [];
        setArticles(data.slice(0, 3).map((a: ApiNews) => ({
          id: a._id,
          title: a.title,
          slug: a.slug,
          category: a.category,
          ...
          authorImage: a.author?.avatar?.url ?? `https://i.pravatar.cc/50?u=${a._id}`,
      ...
      .catch(() => {
        // Use static fallback
        setArticles(staticArticles.slice(0, 3).map((a) => ({
          id: a.id,
          title: a.title,
          slug: a.slug,
          category: a.category,
  ...
  // While loading, use static data to avoid blank flash
  const display = articles.length > 0 ? articles : staticArticles.slice(0, 3).map((a) => ({
    id: a.id, title: a.title, slug: a.slug, category: a.category, excerpt: a.excerpt,
    image: a.image, author: a.author, authorImage: a.authorImage, publishedAt: a.publishedAt, readingTime: a.readingTime,
  }));
    ...
    <section className="section-padding bg-white dark:bg-gray-950">
      <div className="container-custom">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-12">
          <SectionTitle tag="News & Updates" title="Latest From MediSource" align="left" />
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-center gap-3 text-xs text-gray-400 mb-3">
                  <span className="flex items-center gap-1"><Calendar size={12} /> {formatDateShort(article.publishedAt)}</span>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">{article.author}</span>
                  </div>
                  <Link to={`/news/${article.slug}`} className="text-primary-600 dark:text-primary-400 text-sm font-semibold flex items-center gap-1 hover:gap-2 transition-all">
